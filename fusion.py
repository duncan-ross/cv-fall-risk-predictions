...
argparser.add_argument("--video_model_name", type=str, default="openposeMC")
...
if __name__ == "__main__":
    ...
    if args.video_model_name == "openposeMC":
        model1 = OpenPoseMC(num_outputs=len(responses), H=H, W=W, device=device, freeze=True)
    elif args.video_model_name == "resnetMC":
        model1 = ResNetMC(num_outputs=len(responses), H=H, W=W, device=device, freeze=True)
    else:
        raise ValueError("Video model name not recognized")
