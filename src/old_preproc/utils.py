...
def get_framerate(filepath, videoid):
    try:
        res = subprocess.check_output(
            "ffprobe -v error -select_streams v -of default=noprint_wrappers=1:nokey=1 -show_entries stream=r_frame_rate {}".format(
                filepath
            ).split(
                " "
            )
...
if os.path.exists("videometa.pkl"):
    ...
    with open("data/links.csv", "r") as f:
        ...
        for lines in csvFile:
            videoid = lines[0]
            filename = "data/videos/raw/" + lines[1].split("/")[-1]
            videometa[videoid] = {
...
def json2np(json_dir, subjectid):
    ...
    for frame in range(n):
        test_image_json = "{}/input_{}_keypoints.json".format(
...
def fill_nan(A):
    ...
    f = interpolate.interp1d(
        inds[good], A[good], kind="linear", bounds_error=False, fill_value="extrapolate"
    )
...
def center_ts(res):
    ...
    scale = (res[:, (NECK * 3) : (NECK * 3 + 3)] - res[:, (MHIP * 3) : (MHIP * 3 + 3)])[
        :, :2
    ]
    ...
    for i in range(25):
        res[:, (i * 3) : (i * 3 + 3)] = (
            res[:, (i * 3) : (i * 3 + 3)] - np.hstack([X, Y, 0])[None, :]
        )
...
def get_angle_stats(
    A, B, C, res, breaks, framerate=30, name=None, alternate=0, breaks_alt=None
):
    ...
    if name == "trunk_lean" and (alternate == 1) and MORE_PLOTS:
        ...
        plt.plot(
            grid, ts[breaks[1] : breaks[2]] * 180 / np.pi, linestyle="-", linewidth=2.5
        )
        #        plt.axvline(x=(breaks_alt[1] - breaks[1])/framerate, linewidth=2, color='g', linestyle="--")
        plt.axvline(
            x=(breaks[2] - breaks[1]) / framerate,
            linewidth=2,
            color="r",
            linestyle="--",
        )
        plt.axvline(
            x=(breaks[1] - breaks[1]) / framerate,
            linewidth=2,
            color="r",
            linestyle="--",
        )
...
def get_joint_speed(joint, res):
    n = res.shape[0]
    return (
        res[1:n, (joint * 3) : (joint * 3 + 3)]
        - res[0 : (n - 1), (joint * 3) : (joint * 3 + 3)]
    )
...
def get_static(res, down, up):
    ...
    if is3d:
        ...
        knee_dist = (
            res[:, RKNE * 3 : (RKNE * 3 + 2)] - res[:, LKNE * 3 : (LKNE * 3 + 2)]
        )
        knee_dist_mag = np.sqrt(np.sum(knee_dist**2, axis=1))
        ...
        height = (
            res[:, RANK * 3 : (RANK * 3 + 2)] + res[:, LANK * 3 : (LANK * 3 + 2)]
        ) / 2 - res[:, NOSE * 3 : (NOSE * 3 + 2)]
        height_mag = np.sqrt(np.sum(height**2, axis=1))
...
def get_speed_stats(joint, res, breaks, framerate=30, name="pelvic", alternate=False):
    ...
    trunk_speed_mag = trunk_speed_mag[
        trunk_speed_mag < np.percentile(trunk_speed_mag, 95)
    ]  # remove outliers

    trunk_acc_mag = np.append(trunk_acc_mag, trunk_acc_mag[-1])
    trunk_acc_mag = trunk_acc_mag[slices]
    trunk_acc_mag = trunk_acc_mag[
        trunk_acc_mag < np.percentile(trunk_acc_mag, 95)
    ]  # remove outliers
...
def get_acceleration_results(res, breaks, framerate=30, alternate=0):
    ...
    results.update(
        get_speed_stats(
            MHIP, res, breaks, name="pelvic", framerate=framerate, alternate=alternate
        )
    )
    results.update(
        get_speed_stats(
            NECK, res, breaks, name="neck", framerate=framerate, alternate=alternate
        )
    )
    return results
...
def get_angle(A, B, C, data):
    ...
    M = dot_products.copy()
    M[np.abs(M) > 1e-5] = det[np.abs(M) > 1e-5] * np.arccos(
        dot_products[np.abs(M) > 1e-5] / norm_products[np.abs(M) > 1e-5]
    )
...
def get_angle3d(A, B, C, data):
    ...
    M[np.abs(M) > 1e-5] = -(
        det[np.abs(M) > 1e-5]
        * np.arccos(dot_products[np.abs(M) > 1e-5] / norm_products[np.abs(M) > 1e-5])
    )
    ...
    if (len(downs) <= 5 and len(ups) == 5) or (len(downs) <= 4 and len(ups) == 4):
        if max(ups) > max(downs):
            downs = np.append(
                downs,
                max(ups)
                + np.argmin(ind_y_smooth[max(ups) : (max(ups) + ups[-1] - ups[-2])]),
            )
...
def run_openpose(path, slug):
    ...
    CMD = 'ffprobe -loglevel error -select_streams v:0 -show_entries stream_tags=rotate -of default=nw=1:nk=1 -i "{}"'.format(
        path
    )
    ...
    if rotate:
        ...
        CMD = "rm {path} ; ffmpeg -y -i {path_tmp} {path}".format(
            path_tmp=path_tmp, path=path
        )
  ...
  --render_pose 0 ; cp -r {dirpath}/keypoints {slug_dir}'.format(
        dirpath=dirpath, filepath=filepath, slug_dir=keypoints_dir + slug
    )
    print(CMD)
    os.system(CMD)
    return CMD
...
def process_subject(subjectid, processed_npy_path="videos/np/", framerate=None):
    ...
    # estimate height
    lengths = (
        res[ups[1] : ups[-2], 3 * NOSE : (3 * NOSE + 2)]
    ...
    results.update(
        get_angles_results(res, allbreaks, framerate=framerate, alternate=-1)
    )

    results.update(get_acceleration_results(res, downs, framerate=framerate))
    results.update(
        get_acceleration_results(res, allbreaks, framerate=framerate, alternate=1)
    )
    results.update(
        get_acceleration_results(res, allbreaks, framerate=framerate, alternate=-1)
    )
    ...
    if kp3d is not None:
        try:
            ...
            results.update(
                get_angles_results(kp3d, allbreaks, framerate=framerate, alternate=1)
            )
            results.update(
                get_angles_results(kp3d, allbreaks, framerate=framerate, alternate=-1)
            )
        ...
        try:
            ...
            results.update(
                get_acceleration_results(
                    kp3d, allbreaks, framerate=framerate, alternate=1
                )
            )
            results.update(
                get_acceleration_results(
